...
def generate_layout(g: ig.Graph):
    pass
...
def visualize_instruction_dag(instruction_dag: InstructionDAG, collective: Collective):
    ...
    def add_node(op: Op, nnodes):
        ...
        if op.inst == Instruction.start:
            ...
            root_owner[str(op.num)] = root_owner[str(last_writer)] #TODO
            ...
            nnodes = add_chunk_nodes(op.send_match.dst.buffer, op.send_match.dst.rank, op.send_match.dst.index, op.send_match.dst.size, op.num, nnodes, read=False)
            ...
            nnodes = add_chunk_nodes(op.dst.buffer, op.dst.rank, op.dst.index, op.dst.size, op.num, nnodes, read=False)
    ...
    for slot, ops in operations.items():
        ...
        while len(frontier) > 0:
            op = frontier[0]
            if all(dep in visited for dep in op.depends): # If all dependencies are already processes
                if op not in visited:
